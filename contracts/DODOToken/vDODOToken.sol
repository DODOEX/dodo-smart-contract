...
contract vDODOToken is InitializableOwnable {
    ...
    function totalSupply() public view returns (uint256 vDODOSupply) {
        ...
        uint256 actualDODO =
            totalDODO.sub(
                _TOTAL_BLOCK_REWARD_.sub(curDistribution.add(_TOTAL_BLOCK_DISTRIBUTION_))
            );
    ...
    function approve(address spender, uint256 vDODOAmount) public canTransfer returns (bool) {
    ...
    function getLatestAlpha() public view returns (uint256 newAlpha, uint256 curDistribution) {
        ...
        if (_TOTAL_STAKING_POWER_ > 0) {
            newAlpha = uint256(alpha).add(
                DecimalMath.divFloor(curDistribution, _TOTAL_STAKING_POWER_)
            );
        } else {
            newAlpha = alpha;
        }
